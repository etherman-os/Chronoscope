...
describe("EventTimeline", () => {
  ...
  it("positions click events in the top half", () => {
    ...
    const { container } = render(
      <EventTimeline events={events} currentTime={0} />,
    );
    const marker = container.querySelector('[title="click — 1000ms"]');
    expect(marker).not.toBeNull();
    if (marker) {
      expect(marker.className).toContain(styles.click);
    }
  });
  ...
  it("positions scroll events in the bottom half", () => {
    ...
    expect(marker).not.toBeNull();
    if (marker) {
      expect(marker.className).toContain(styles.scroll);
    }
  });
  ...
  it("renders current time indicator", () => {
    ...
    const { container } = render(
      <EventTimeline events={events} currentTime={500} />,
    );
  ...
  it("scales events relative to max time", () => {
    ...
    const { container } = render(
      <EventTimeline events={events} currentTime={2500} />,
    );
    const marker = container.querySelector('[title="click — 5000ms"]');
    expect(marker).not.toBeNull();
    if (marker) {
      // Should be at 100% since it's the max time
      expect((marker as HTMLElement).style.getPropertyValue("--left")).toBe(
        "100%",
      );
    }
  });
